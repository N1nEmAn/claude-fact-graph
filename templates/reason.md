...
```json
{"accepted": true, "data": {"intents": [{"title": "复现空token崩溃", "from": ["f001"], "description": "concrete next step", "requires_confirmation": false}]}}
```

**`title`** is REQUIRED on every intent — a short (2–10 字) human-readable label in the
SAME language as the goal (中文 goal → 中文 title). It is what shows up as the node
label in the graph view. Keep it concrete and scannable, e.g. "复现崩溃", "定位空指针",
"补单测". Never repeat the description verbatim.
